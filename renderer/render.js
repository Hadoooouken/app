...
import { UNITS_PER_M } from '../engine/state.js' // export const UNITS_PER_M = 100
...
function readableRotation(deg) {
  ...
  while (r > 180) r -= 360
  while (r < -180) r += 360
  // если “вверх ногами” — переворачиваем
  if (r > 90 || r < -90) r += 180
  return r
}
...
export function render(draw) {
  ...
  // 1) CAPITAL
  for (const w of caps) {
    wallsG.line(w.a.x, w.a.y, w.b.x, w.b.y).stroke({
      width: CAP_W,
      color: CAP_COLOR,
      linecap: 'round',
      linejoin: 'round',
  ...
  if (state.selectedWallId) {
    ...
    if (w) {
      ...
      const r = 12
      wallsG.circle(r).center(w.a.x, w.a.y).fill('#fff')
        .stroke({ width: 3, color: SELECT_COLOR })
        .attr({ 'pointer-events': 'none' })

      wallsG.circle(r).center(w.b.x, w.b.y).fill('#fff')
        .stroke({ width: 3, color: SELECT_COLOR })
        .attr({ 'pointer-events': 'none' })
    }
  }
  ...
  // ожидаем: state.ui.snapPulse = { x, y, t } (t = performance.now() / Date.now())
  {
  ...
  const offsetFromWall = 14 * invScale
  ...
  for (const w of walls) {
    ...
    const bb = t.bbox()
    const bg = dimsG.rect(bb.width + pad * 2, bb.height + pad * 2)
